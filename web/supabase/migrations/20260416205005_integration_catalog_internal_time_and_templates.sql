...
-- promote provenance, and per-catalog integration task templates.
...
create table public.integration_catalog_task_templates (
...
create index integration_catalog_task_templates_integration_idx
  on public.integration_catalog_task_templates (integration_id);

alter table public.integration_catalog_task_templates enable row level security;

create policy "integration_catalog_task_templates_owner"
  on public.integration_catalog_task_templates
  for all
  using (
    exists (
      select 1
      from public.integrations i
      where i.id = integration_catalog_task_templates.integration_id
