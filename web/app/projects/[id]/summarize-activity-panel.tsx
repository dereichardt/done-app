...
/** Inbox inline: parent renders Open project / Generate / Mark done; panel omits footer and recent summaries. */
export type SummarizeActivityEmbedUi = {
  view: PanelView;
  generating: boolean;
  canSubmit: boolean;
  /** Result finished with stream text and no error — show Mark done in parent. */
  summaryReady: boolean;
  hasError: boolean;
};

export type SummarizeActivityPanelHandle = {
...
export const SummarizeActivityPanel = forwardRef<SummarizeActivityPanelHandle, SummarizeActivityPanelProps>(
  function SummarizeActivityPanel(
    {
  projectId,
  projectCustomerName,
  variant = "modal",
  ...
  embedInbox = false,
  onEmbedUiChange,
}: SummarizeActivityPanelProps,
    ref,
  ) {
  const abortRef = useRef<AbortController | null>(null);

  const [view, setView] = useState<PanelView>("picker");
  ...
  useEffect(() => {
    if (embedInbox) return;
    if (view === "picker") void refreshHistory();
  }, [view, refreshHistory, embedInbox]);
  ...
  useImperativeHandle(
    ...
    [handleBackToPicker, startGeneration],
  );

  const summaryReady =
    view === "result" && active.done && !active.error && active.text.trim().length > 0;
  const hasError = view === "result" && Boolean(active.error);

  const onEmbedUiChangeRef = useRef(onEmbedUiChange);
  onEmbedUiChangeRef.current = onEmbedUiChange;

  useEffect(() => {
    if (!embedInbox) return;
    onEmbedUiChangeRef.current?.({
      view,
      generating,
      canSubmit,
      summaryReady,
  ...
  }, [embedInbox, view, generating, canSubmit, summaryReady, hasError]);
  ...
  const showFooterCancel = Boolean(onDismiss);
  const showInternalFooter = !(variant === "embedded" && embedInbox);

  return (
    <div
      className={
        variant === "modal"
          ? "flex h-full max-h-[inherit] min-h-0 flex-col"
          : "flex min-h-0 flex-1 flex-col"
      }
      style={variant === "modal" ? { maxHeight: "inherit" } : undefined}
    >
      {variant === "modal" ? (
        <div
      ...
      ) : variant === "embedded" && embedInbox ? null : (
        <p className="shrink-0 px-4 pt-1 text-sm" style={{ color: "var(--app-text-muted)" }}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {view === "picker" ? (
          <PickerView
            ...
            onToggleHistory={(id) => setExpandedHistoryId((prev) => (prev === id ? null : id))}
            hideRecentSummaries={embedInbox}
          />
        ) : (
      ...
      {showInternalFooter ? (
        ...
        >
          {view === "picker" ? (
            <>
              {showFooterCancel ? (
                <button type="button" className="btn-cta text-xs" onClick={onDismiss}>
                  Cancel
                </button>
              ) : null}
              <button
...
SummarizeActivityPanel.displayName = "SummarizeActivityPanel";
    ...
    <div className="flex flex-col gap-3 p-4">
      ...
      {active.done && !active.error ? (
        <p className="mt-2 text-[11px]" style={{ color: "var(--app-text-muted)" }}>
          Built from recorded activity · saved for {SUMMARY_EXPIRY_DAYS} days.
        </p>
      ) : null}
    </div>
  );
